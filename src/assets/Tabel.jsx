...
function Tabel() {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_LINK}/route/products`)
      .then(res => res.json())
  ...
  const deleteUser = async (id) => {
    try {
      const res = await fetch(`${process.env.REACT_APP_LINK}/route/delete/${id}`, {
        method: "DELETE",
      });
              ...
              return (
                <tr key={_id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                  ...
                  <td className="px-6 py-4">
                    <img src={`${process.env.REACT_APP_LINK}/${media}`} alt={media} className="w-[30px] h-[30px]" />
                  </td>
